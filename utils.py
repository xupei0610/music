...
@torch.jit.script
def euler_xyz(q: torch.Tensor, epsilon: float=0):
    q1, q2, q3, q0 = torch.unbind(q, -1)
...
@torch.jit.script
def expmap2quat(expmap: torch.Tensor) -> torch.Tensor:
    angle = expmap.norm(p=2, dim=-1, keepdim=True)
    axis = expmap / angle.clamp(min=1e-9)
    original_shape = list(axis.shape)[:-1]
    axis = axis.view(-1, 3)
    angle = angle.view(-1)
    quat = axang2quat(axis, angle)
    return quat.view(original_shape + [4])
...
@torch.jit.script
def quat2expmap2(w: torch.Tensor, xyz: torch.Tensor) -> torch.Tensor:
    sin = torch.sqrt(1 - w * w)
    mask = sin > 1e-5
    ...
    ang = torch.where(mask, angle, z_axis[...,0])
    ax = torch.where(mask.unsqueeze_(-1), axis, z_axis)
